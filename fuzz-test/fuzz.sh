...
build_targets() {
    ...
    cmake \
        ...
        -DCMAKE_C_FLAGS="-O9 -std=c99 -Wall -Wextra -Wpedantic -Wno-gnu-statement-expression -DAFL_FAST -DFUZZ_TOOL=2" .. && \
    make -j && \
    cd .. && \
    unset AFL_HARDEN && \
    ...
    cd $debug_build_dir && \
    cmake \
        -DCMAKE_C_FLAGS="-O0 -g -ggdb -std=c99 -Wall -Wextra -Wpedantic -fprofile-arcs -ftest-coverage -DFUZZ_TOOL=3" \
...
do_cmin_libfuzzer() {
    ...
    ./$binary_dir/$target $output_dir/queue_cmin $output_dir/queue_all -merge=1
...
do_continue_fuzz_afl() {
    ...
    screen -dmS $masterFuzzer afl-fuzz -x fuzz.dict $fuzz_args -i $output_dir/queue_all -o $output_dir -M fuzzer1 -- ./$binary_dir/$target
    sleep 1

    if ! screen -list | grep -q $masterFuzzer; then
        echo "Error: Master fuzzing start failed."
        afl-fuzz -x fuzz.dict $fuzz_args -i $output_dir/queue_all -o $output_dir -M fuzzer1 -- ./$binary_dir/$target
        exit 1
    fi

    for a in $(seq 2 $num_cores); do
        slaveFuzzer=$a"Slave$target"
        screen -dmS $slaveFuzzer afl-fuzz -x fuzz.dict $fuzz_args -i $output_dir/queue_all -o $output_dir -S fuzzer$a -- ./$binary_dir/$target
    done
